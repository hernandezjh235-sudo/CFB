...
def render_moneyline_nfl(g:dict,ctx:Dict[str,dict]):
    ...
    al=str(g.get('away_logo') or _logo(ctx,away)); hl=str(g.get('home_logo') or _logo(ctx,home)); fl=(al if fav==away else hl) or _logo(ctx,fav)
...
def render_player_nfl(r:dict,ctx:Dict[str,dict],game:dict,rank:int=1):
    team=str(r.get('team') or ''); opp=str(r.get('opp') or ''); game=game or {}; logo=_logo(ctx,team) or (str(game.get('away_logo') or '') if _norm(team)==_norm(game.get('away')) else str(game.get('home_logo') or '') if _norm(team)==_norm(game.get('home')) else ''); color=_color(ctx,team); side=str(r.get('side') or 'Over'); status=str(r.get('status') or 'TRACK')
...
def render_fast_rows(rows:List[dict],ctx:Dict[str,dict],limit:int=60):
    ...
    for i,r in enumerate(rows[:limit],1):
        team=str(r.get('team') or ''); game=r.get('_game') or {}; logo=_logo(ctx,team) or (str(game.get('away_logo') or '') if _norm(team)==_norm(game.get('away')) else str(game.get('home_logo') or '') if _norm(team)==_norm(game.get('home')) else ''); color=_color(ctx,team); opp=str(r.get('opp') or '')
