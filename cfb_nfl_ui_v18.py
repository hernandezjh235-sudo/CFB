...
def render_moneyline_nfl(g:dict,ctx:Dict[str,dict]):
    ...
    st.markdown(f'''<section class="cfb18-ml" style="--away:{ac};--home:{hc};--winner:{fc}"><div class="cfb18-top"><span>{html.escape(away)} @ {html.escape(home)}</span><span>CFB MONEYLINE · MODEL</span></div><div class="cfb18-mlgrid"><div class="cfb18-team">{im(al,away)}<div class="cfb18-teamname">{html.escape(_abbr(ctx,away))}</div><div class="cfb18-points">{_fmt(g.get('away_points'))}</div><div class="cfb18-prob">{awp:.1f}% WIN</div></div><div class="cfb18-win"><div class="cfb18-winkicker">★ MODEL WINNER ★</div>{im(fl,fav)}<div class="cfb18-winname">{html.escape(fav)}</div><div class="cfb18-winprob">{fwp:.1f}%</div><div class="cfb18-prob">Projected margin {_fmt(abs(_num(g.get('model_home_margin'))))}</div></div><div class="cfb18-team">{im(hl,home)}<div class="cfb18-teamname">{html.escape(_abbr(ctx,home))}</div><div class="cfb18-points">{_fmt(g.get('home_points'))}</div><div class="cfb18-prob">{hwp:.1f}% WIN</div></div></div><div class="cfb18-mlmetrics"><div class="cfb18-mini"><div class="cfb18-lab">Model Spread</div><div class="cfb18-val">{(-_num(g.get('model_home_margin'))):+.1f}</div></div><div class="cfb18-mini"><div class="cfb18-lab">Model Total</div><div class="cfb18-val">{_fmt(g.get('model_total'))}</div></div><div class="cfb18-mini"><div class="cfb18-lab">Spread Edge</div><div class="cfb18-val">{spread_edge}</div></div><div class="cfb18-mini"><div class="cfb18-lab">Total Edge</div><div class="cfb18-val">{total_edge}</div></div></div><div class="cfb18-tags">{html.escape(tags)}</div></section>''',unsafe_allow_html=True)
...
def render_player_nfl(r:dict,ctx:Dict[str,dict],game:dict,rank:int=1):
    ...
    st.markdown(f'''<section class="cfb18-card" style="--team:{color}"><div class="cfb18-ident"><div class="cfb18-rank">#{rank}</div>{logo_html}<div><div class="cfb18-name">{html.escape(str(r.get('player') or ''))}</div><div class="cfb18-sub">{html.escape(team)} VS {html.escape(opp)} · {html.escape(str(r.get('matchup') or game.get('away','')+' @ '+game.get('home','')))}</div></div><div class="cfb18-status">{html.escape(status)}</div></div><div class="cfb18-market"><div class="cfb18-main"><div class="cfb18-lab">{html.escape(str(r.get('prop') or 'PROP'))}</div><div class="cfb18-val">{html.escape(side.upper())} {_fmt(r.get('line'))}</div></div><div class="cfb18-stat"><div class="cfb18-lab">Projection</div><div class="cfb18-val">{_fmt(r.get('projection'))}</div></div><div class="cfb18-stat"><div class="cfb18-lab">Edge</div><div class="cfb18-val">{_num(r.get('edge')):+.1f}</div></div><div class="cfb18-stat"><div class="cfb18-lab">Likely</div><div class="cfb18-val cfb18-probval">{_pct(r.get('probability')):.1f}%</div></div><div class="cfb18-stat"><div class="cfb18-lab">Model SD</div><div class="cfb18-val">{_fmt(r.get('sd'))}</div></div></div><div class="cfb18-why"><b>GAME:</b> {html.escape(str(game.get('away') or ''))} {_fmt(game.get('away_points'))} — {_fmt(game.get('home_points'))} {html.escape(str(game.get('home') or ''))} · Total {_fmt(game.get('model_total'))}<br><b>WHY:</b> {notes}</div></section>''',unsafe_allow_html=True)
...
def render_fast_rows(rows:List[dict],ctx:Dict[str,dict],limit:int=60):
    ...
    for i,r in enumerate(rows[:limit],1):
        ...
        lg=f"<img src='{html.escape(logo,quote=True)}' alt='{html.escape(team)} logo' loading='lazy' onerror=\"this.style.display='none'\">" if logo else '<div></div>'
