...
def render_player_nfl(r:dict,ctx:Dict[str,dict],game:dict,rank:int=1):
    ...
    st.markdown(f'''<section class="cfb18-card" style="--team:{color}"><div class="cfb18-ident"><div class="cfb18-rank">#{rank}</div>{logo_html}<div><div class="cfb18-name">{html.escape(str(r.get('player') or ''))}</div><div class="cfb18-sub">{html.escape(team)} VS {html.escape(opp)} · {html.escape(str(r.get('matchup') or game.get('away','')+' @ '+game.get('home','')))}</div></div><div class="cfb18-status">{html.escape(status)}</div></div><div class="cfb18-market"><div class="cfb18-main"><div class="cfb18-lab">{html.escape(str(r.get('prop') or 'PROP'))}</div><div class="cfb18-val {'cfb18-side-over' if side.lower()=='over' else 'cfb18-side-under'}">{html.escape(side.upper())} {_fmt(r.get('line'))}</div></div><div class="cfb18-stat"><div class="cfb18-lab">Projection</div><div class="cfb18-val">{_fmt(r.get('projection'))}</div></div><div class="cfb18-stat"><div class="cfb18-lab">Edge</div><div class="cfb18-val">{_num(r.get('edge')):+.1f}</div></div><div class="cfb18-stat"><div class="cfb18-lab">Likely</div><div class="cfb18-val cfb18-probval">{_pct(r.get('probability')):.1f}%</div></div><div class="cfb18-stat"><div class="cfb18-lab">Model SD</div><div class="cfb18-val">{_fmt(r.get('sd'))}</div></div></div><div class="cfb18-why"><b>GAME:</b> {html.escape(str(game.get('away') or ''))} {_fmt(game.get('away_points'))} — {_fmt(game.get('home_points'))} {html.escape(str(game.get('home') or ''))} · Total {_fmt(game.get('model_total'))}<br><b>WHY:</b> {notes}</div></section>''',unsafe_allow_html=True)
