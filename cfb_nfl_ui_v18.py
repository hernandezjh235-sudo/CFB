...
def render_moneyline_nfl(g:dict,ctx:Dict[str,dict]):
    ...
    aid=str(g.get('away_espn_id') or ''); hid=str(g.get('home_espn_id') or '')
...
def render_player_nfl(r:dict,ctx:Dict[str,dict],game:dict,rank:int=1):
    team=str(r.get('team') or ''); opp=str(r.get('opp') or ''); game=game or {}; _isaway=_norm(team)==_norm(game.get('away')); _ishome=_norm(team)==_norm(game.get('home')); _eid=str(game.get('away_espn_id') or '') if _isaway else str(game.get('home_espn_id') or '') if _ishome else ''; logo=(str(game.get('away_logo') or '') if _isaway else str(game.get('home_logo') or '') if _ishome else '') or (f'https://a.espncdn.com/i/teamlogos/ncaa/500/{_eid}.png' if _eid else '') or _logo(ctx,team); color=_color(ctx,team); side=str(r.get('side') or 'Over'); status=str(r.get('status') or 'TRACK')
...
def render_fast_rows(rows:List[dict],ctx:Dict[str,dict],limit:int=60):
    ...
    for i,r in enumerate(rows[:limit],1):
        team=str(r.get('team') or ''); game=r.get('_game') or {}; _isaway=_norm(team)==_norm(game.get('away')); _ishome=_norm(team)==_norm(game.get('home')); _eid=str(game.get('away_espn_id') or '') if _isaway else str(game.get('home_espn_id') or '') if _ishome else ''; logo=(str(game.get('away_logo') or '') if _isaway else str(game.get('home_logo') or '') if _ishome else '') or (f'https://a.espncdn.com/i/teamlogos/ncaa/500/{_eid}.png' if _eid else '') or _logo(ctx,team); color=_color(ctx,team); opp=str(r.get('opp') or '')
